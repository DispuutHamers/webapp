class Api2::EventsController < Api2::ApiController
  ...
  def index
    puts params[:future] + " DEBUG"
    if params[:future] == "true"
      events = Event.where(date: Date.current..(Date.current+15.years)) # Works with ranges for future scopes
    else
      events = Event.all
    end
    ...
    if params[:sorted] == 'date-asc'
      ...
      render json: events.order('date').reverse_order
    else
      render json: events
    end
  end
